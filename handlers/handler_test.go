...
func TestNew_CreatesHandler(t *testing.T) {
	...
	// Act
	handler := handlers.New(handlers.HandlerConfig{IDEncoder: encoder, ResponseHandler: responseHandler})
...
func TestEncodeID_Success(t *testing.T) {
	// Arrange
	encoder := createTestEncoder()
	handler := handlers.New(handlers.HandlerConfig{IDEncoder: encoder})
