...
[[ -d "/var/log/motogo" ]] && echo "  ✓ Logs del backend: /var/log/motogo" || echo "  ✗ Logs no montados"
...
run_logrotate() {
    echo "[[$(date "$LOG_DATE_FMT")]] Ejecutando logrotate..."
...
run_volume_cleanup() {
    echo "[[$(date "$LOG_DATE_FMT")]] Ejecutando limpieza de volúmenes..."
...
(
  while true; do
    sleep 3600  # 1 hora
    echo "[[$(date "$LOG_DATE_FMT")]] Ejecutando logrotate programado..." >> /var/log/logrotate.log
...
(
  while true; do
    sleep 21600  # 6 horas
    echo "[[$(date "$LOG_DATE_FMT")]] Log rotator is alive" >> /var/log/logrotate.log
  done
) &
